...
class SideViewProcessor:
    ...
    def _determine_contrast_color(self, image: np.ndarray, landmarks: List) -> int:
        ...
        if len(points) < 4:
            return 200  # Default to light gray
        ...
        # Dark clothing (< 100) -> light background (220)
        # Light clothing (> 150) -> dark background (60)
        # Medium clothing -> gray background (128)
...
def main():
    processor = SideViewProcessor()
    result = processor.process("side_view.png", "processed_side_view.png")
